...
func TestNInWordsDays(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "00", args: args{n: 0, showZero: false, withZeros: false}, want: ""},
		{name: "01", args: args{n: 0, showZero: false, withZeros: true}, want: ""},
		{name: "02", args: args{n: 0, showZero: true, withZeros: false}, want: "ноль дней"},
		{name: "03", args: args{n: 0, showZero: true, withZeros: true}, want: "ноль дней"},
		...
		{name: "1000040", args: args{n: 1000040, withZeros: true}, want: "один миллион ноль тысяч сорок дней"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
